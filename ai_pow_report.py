...
const pct=x=>x===null||x===undefined?unknown:Math.round(Number(x)*100)+'%';
...
let resizeTimer;
addEventListener('resize',()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(chart,150)});
...
$('#content').innerHTML=`
<div class="tabs" role="group" aria-label="Report view">
  <button id="tab-latest" aria-pressed="false">Latest commit</button>
  <button id="tab-iteration" aria-pressed="false" ${total?'':'disabled'}>Project iterations</button>
</div>
...
<section id="view-iteration" hidden>
  ...
  <div class="section">
    ...
    <div class="chart-box"><div class="chart-top"><span>Project total</span><span>${esc(total.algorithm)}</span></div><div id="chart"></div></div>
  </div>
...
$('#tab-iteration').addEventListener('click',()=>setView('iteration'));
if(total){journal();$('#grade-filter').addEventListener('change',journal)}
