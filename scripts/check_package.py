...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aipow-package-") as tmp:
        ...
        assert result.stdout.strip() == "0.5.0", result.stdout
        subprocess.run([str(python), "-c", "import ai_pow, ai_pow_scoring, ai_pow_report; assert ai_pow.APP_VERSION == '0.5.0'; assert ai_pow_scoring.ALGORITHM == 'retention-v2'; assert callable(ai_pow_report.render)"],
